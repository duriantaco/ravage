...
class _AdaptiveClient:
    def complete(
        ...
        *,
        ...
        if len(messages) == 2:
        ...
        elif candidate_assisted and len(messages) == 4:
            action = {
                "action": "excerpt",
                "args": {"path": "app.py", "start_line": 5, "end_line": 20},
            }
            ...
            if candidate_assisted:
                ...
                findings.append(
                    {
                        "vuln_class": "sql_injection",
                        "title": "Duplicate evaluated-class finding",
                        "severity": "low",
                        "confidence": "low",
                        "description": "This tests class-conditioned false positives.",
                        "recommendation": "Deduplicate findings that cite the same weakness.",
                        "evidence_ids": ["excerpt-1"],
                    }
                )
                findings.append(
                    {
                        "vuln_class": "xss",
                        "title": "Off-class diagnostic finding",
                        "severity": "low",
                        "confidence": "low",
                        "description": "This tests separate off-class accounting.",
                        "recommendation": "Inspect output encoding separately.",
                        "evidence_ids": ["excerpt-1"],
                    }
                )
            action = {
...
def test_ab_runner_repeats_both_arms_and_aggregates_detection_and_telemetry(
    ...
    assert assisted_score["false_positives"] == 2
    assert assisted_score["false_negatives"] == 0
    assert report["detection"]["off_class_unscored_findings"] == 2
    assert arms[LITERAL_ONLY_ARM]["detection"]["off_class_unscored_findings"] == 0
    assert arms[CANDIDATE_ASSISTED_ARM]["detection"]["off_class_unscored_findings"] == 2
    assert report["tools"]["context_actions"] == 6
    ...
    assert report["model"] == {
        ...
        "requested_actions": {"excerpt": 4, "final": 4, "list_source_candidates": 2},
    }
    ...
    assert stratum["detection"]["off_class_unscored_findings"] == 2
    ...
    assert stratum_arms[CANDIDATE_ASSISTED_ARM]["detection"]["off_class_unscored_findings"] == 2
    comparison = report["paired_comparison"]
    assert comparison["inference"] == "descriptive_only_no_significance_test"
    assert comparison["completed_pairs"] == 2
    assert comparison["comparable_pairs"] == 2
    assert comparison["incomplete_pairs"] == 0
    assert set(comparison["metric_pair_counts"].values()) == {2}
    assert comparison["mean_deltas"] == {
        "completion": 0.0,
        "true_positives": 1.0,
        "false_positives": 1.0,
        "false_negatives": -1.0,
    ...
    assert comparison["win_tie_loss"]["true_positives"] == {
        "candidate_preference": "higher",
        "candidate_wins": 2,
        "ties": 0,
        "candidate_losses": 0,
    }
    assert comparison["win_tie_loss"]["cost_usd"] == {
        "candidate_preference": "lower",
        "candidate_wins": 0,
        "ties": 0,
        "candidate_losses": 2,
    }
    assert comparison["win_tie_loss"]["false_positives"] == {
        "candidate_preference": "lower",
        "candidate_wins": 0,
        "ties": 0,
        "candidate_losses": 2,
    }
    assert comparison["win_tie_loss"]["context_actions"] == {
        "candidate_preference": "lower",
        "candidate_wins": 0,
        "ties": 0,
        "candidate_losses": 2,
    }
    assert comparison["pairs"][0]["deltas"] == {
        "completion": 0,
        "true_positives": 1,
        "false_positives": 1,
        "false_negatives": -1,
...
def test_failed_runs_keep_actual_reply_cost_and_stop_at_aggregate_ceiling(
    ...
    assert comparison["incomplete_pairs"] == 2
    assert comparison["metric_pair_counts"]["completion"] == 2
    assert comparison["metric_pair_counts"]["true_positives"] == 0
    assert comparison["mean_deltas"]["completion"] == 0.0
    assert comparison["mean_deltas"]["true_positives"] is None
    assert all(pair["deltas"] == {"completion": 0} for pair in comparison["pairs"])
...
def test_runner_rejects_a_completed_result_after_fixture_snapshot_changes(
    ...
    assert second["model"]["replies"] == 3
    comparison = report["paired_comparison"]
    assert comparison["completed_pairs"] == 0
    assert comparison["comparable_pairs"] == 0
    assert comparison["pairs"][0]["deltas"] == {"completion": -1}
